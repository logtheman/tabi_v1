...
import Activity from "./Activity_dc";
...
const DayTimeLine = props => {
	...
	let dayTimeline = props.dayInfo.map((act, i) => {
		let space = i > 0
			...
			: 0; //find difference in hours between activities and add space after the first activity
		if (i !== 0) {
			return (
				<div
					key={i}
					onMouseEnter={() => props.handleMouseEnterActivity(i)}
					onMouseLeave={props.handleMouseLeaveActivity}
				>
